// src/components/CustomBarChartWidget.tsx
import React, { useState, useMemo } from 'react';
import { ChartWidget, type BeforeRenderHandler } from '@sisense/sdk-ui';
import { measureFactory, CalculatedMeasure, Attribute } from '@sisense/sdk-data';
...
const CustomBarChartWidget: React.FC = () => {
  const { theme } = useTheme();
  const [activeFilter, setActiveFilter] = useState<typeof THRESHOLDS[number] | null>(null);
  ...
  const conditionalMeasure: CalculatedMeasure = useMemo(() => {
    ...
    if (!activeFilter) {
      ...
      if (activeFilter.name === 'Minimal') {
        ...
        const thresholdIndex = THRESHOLDS.findIndex(t => t.name === activeFilter.name);
  ...
  const handleBeforeRender: BeforeRenderHandler = (options) => {
    return Highcharts.merge(true, options, modifyChartStyle(options));
  };
    ...
    <div className="custom-barchart-widget">
      ...
      <div className="color-filter-controls">
        ...
        {THRESHOLDS.map(t => (
            <button
              key={t.name}
              className={`filter-button ${activeFilter?.name === t.name ? 'active' : ''}`}
