...
import { useTheme, type Theme } from './ThemeService';
...
// Memoize your custom widgets to prevent unnecessary re-renders
...
const App: FC = () => {
  ...
  const handleUpdateFolder = async (id: string, newName: string, newColor?: string) => {
    const { error } = await supabase.from('dashboards').update({ name: newName, color: newColor }).eq('id', id);
  ...
  const isChartWidget = (widgetId: string | null): boolean => {
    ...
    return instance.id.startsWith('chart') || instance.id === 'styled-embed' || instance.id === 'average-leak-rate-widget';
  };
