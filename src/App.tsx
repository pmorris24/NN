...
import CustomBarChartWidget from './components/CustomBarChartWidget';
import AverageLeakRateWidget from './components/AverageLeakRateWidget';
import MapWidget from './components/MapWidget';
import LeakDistributionPieChart from './components/LeakDistributionPieChart';
import ScatterPlotWidget from './components/ScatterPlotWidget';
import IndicatorCardWidget from './components/IndicatorCardWidget';
...
const App: FC = () => {
  ...
  const MemoizedCustomBarChartWidget = React.memo(CustomBarChartWidget);
  const MemoizedAverageLeakRateWidget = React.memo(AverageLeakRateWidget);
  const MemoizedMapWidget = React.memo(MapWidget);
  const MemoizedPieChartWidget = React.memo(LeakDistributionPieChart);
  const MemoizedScatterPlotWidget = React.memo(ScatterPlotWidget);
  const MemoizedIndicatorCardWidget = React.memo(IndicatorCardWidget);
  const MemoizedTimeSeriesWidget = React.memo(TimeSeriesWidget);
  ...
  const openEditorForWidget = (widgetId: string) => {
    const widget = widgetInstances.find((w) => w.instanceId === widgetId);
    if (widget) {
  ...
  const handleSaveDashboardUpdate = async () => {
    ...
    const dashboardToUpdate = dashboards.find(
    ...
    if (dashboardToUpdate) {
      ...
      if (error) console.error('Error updating dashboard:', error);
        ...
        setDashboards((prev) =>
          prev.map((d) => (d.id === activeDashboardId ? updatedDashboard : d))
        );
  ...
  const isChartWidget = (widgetId: string | null): boolean => {
    ...
    return (
      instance.id.startsWith('chart') ||
      instance.id === 'styled-embed' ||
      instance.id === 'average-leak-rate-widget'
    );
  };

  const currentlyEditingWidget = widgetInstances.find(
    (w) => w.instanceId === editingWidgetId
  );
  const currentlyEditingEmbed = isEmbedModalOpen.instanceId
    ? widgetInstances.find((w) => w.instanceId === isEmbedModalOpen.instanceId)
    : null;
  ...
  const onBeforeRenderStyledWidget = (
    ...
    if (options.plotOptions) {
      if (options.plotOptions.series) {
        options.plotOptions.series.opacity = styleConfig.barOpacity;
      }
      if (options.plotOptions.pie) {
        options.plotOptions.pie.opacity = styleConfig.pieOpacity;
      }
    }
  ...
  const currentlyEditingWidgetForColors = widgetInstances.find(
    (w) => w.instanceId === editingWidgetId
  );
      ...
      <ThemeProvider theme={theme === 'dark' ? darkTheme : lightTheme}>
        <div className="app-root">
          <Header
            ...
            onToggleAnalytics={() =>
              showIframeView(
                `${sisenseUrl}/app/main/home?embed=true&edit=true&r=false&l=false&t=false`,
                'analytics'
              )
            }
            onToggleAdmin={() =>
              showIframeView(
                `${sisenseUrl}/app/settings?embed=true&edit=true&r=true&l=true&t=true`,
                'admin'
              )
            }
            onToggleData={() =>
              showIframeView(
                `${sisenseUrl}/app/main/data?embed=true&edit=true&r=false&l=false&t=false`,
                'data'
              )
            }
            onTogglePulse={() =>
              showIframeView(
                `${sisenseUrl}/app/main/pulse?embed=true&edit=true&r=false&l=false&t=false`,
                'pulse'
              )
            }
            onProfileClick={() =>
              showIframeView(`${sisenseUrl}/app/profile`, null)
            }
            ...
            <div className="content-wrapper">
              {iframeUrl ? (
                ...
                <div className="dashboard-content">
                  ...
                  <div ref={layoutRef} className="layout-wrapper">
                    ...
                    >
                      {widgetInstances.map((w) => {
                        ...
                        return (
                          ...
                          >
                            {w.id === 'custom-barchart-widget' ? (
                              <MemoizedCustomBarChartWidget />
                            ) : w.id === 'average-leak-rate-widget' ? (
                              <MemoizedAverageLeakRateWidget />
                            ) : w.id === 'map-widget' ? (
                              <MemoizedMapWidget />
                            ) : w.id === 'leak-distribution-pie-chart' ? (
                              <MemoizedPieChartWidget />
                            ) : w.id === 'scatter-plot-widget' ? (
                              <MemoizedScatterPlotWidget />
                            ) : w.id === 'indicator-card-widget' ? (
                              <MemoizedIndicatorCardWidget />
                            ) : w.id === 'time-series-widget' ? (
                              <MemoizedTimeSeriesWidget />
                            ) : w.id === 'styled-embed' &&
                              w.widgetOid &&
                              w.dashboardOid ? (
                              <DashboardWidget
            ...
            currentlyEditingWidgetForColors.series && (
              <Modal
                ...
                title={`Editing Colors: ${
                  currentlyEditingWidgetForColors.title || 'Widget'
                }`}
              >
                <ChartColorStyler
